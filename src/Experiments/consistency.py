...
class Consistency(object):
    def __init__(self, results_folder=None, file_out=None, trec='task', b=None):
        self.self = self
    ...
    def calc_corr_by_topic(self, file_out=None):
        ...
        for b in range(self.b):
            new_list = self.queries[:]
            # shuffle the new list
            random.shuffle(new_list)
            if app == 'split':
                splits_queries = np.array_split(new_list, 2)
        ...
        #uncomment this if you want to print the results
        # self.print_average_corr(self.dict_correlations_by_topic)
    ...
    def consistency(self):
        self.queries = list(self.read_run(self.path_models[0]).keys())
        dict_b = {}
        for b in range(self.b):
            new_list = self.queries[:]
            # shuffle the new list
            ...
            self.dict_of_models_order1 = {}
            self.dict_of_models_order2 = {}
            for model, path in zip(self.models, self.path_models):
                data = pd.read_csv(path, encoding="iso-8859-1", sep=",")
                df = pd.DataFrame(data=data)
                df1 = df.loc[df['qid'].isin(splits_queries[0])]
                df2 = df.loc[df['qid'].isin(splits_queries[1])]
                dict_average1 = {colum: df1[colum].mean() for colum in df1.columns[1:]}
                dict_average2 = {colum: df2[colum].mean() for colum in df2.columns[1:]}

                for metric in dict_average1:
                    if metric not in dict_b:
                        dict_b[metric] = []
                    if metric not in self.dict_of_models_order1:
                        self.dict_of_models_order1[metric] = {}
                        self.dict_of_models_order2[metric] = {}
                    if model not in self.dict_of_models_order1[metric]:
                        self.dict_of_models_order1[metric][model] = {}
                        self.dict_of_models_order2[metric][model] = {}
                    self.dict_of_models_order1[metric][model] = dict_average1[metric]
                    self.dict_of_models_order2[metric][model] = dict_average2[metric]


            for metric in self.dict_of_models_order1:
                a = sorted(self.dict_of_models_order1[metric], key=self.dict_of_models_order1[metric].get)
                # print(a[0])
                b = sorted(self.dict_of_models_order2[metric], key=self.dict_of_models_order2[metric].get)
                # print(b[0])
                tau, p_value = stats.kendalltau(a, b)
                dict_b[metric] += [tau]
        # pprint.pprint(dict_b)
        averages = {}
        print(self.path_models[0])
        for metric in ['CAM','MM','euclidean','manhattan','chebyshev']:
            if metric not in averages:
                averages[metric] = np.mean(dict_b[metric])
            print(round(averages[metric], 4))
