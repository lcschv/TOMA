...
for measure in ['AP','NDCG']:
    for track in ['adhoc']:
        ...
        if measure == 'AP':
            folder = "data/results_weighting_cutoff100_ap/Decision/"
        else:
            folder = "data/results_weighting_cutoff100/Decision/"
        # instantiation = "AP"
        ...
        results_folder = "data/results_weighting/top_100_per_track_perquery_cutoff/Decision2019/"
        # results_folder = "data/results_weighting/top_100_per_track_perquery_cutoff/Misinfo-"+track+'/'
        ...
        metrics_to_print = {'CAM':'CAM','MM':'MM','euclidean':'EUCL','manhattan':'MANH','chebyshev':'CHEB',
                            'skyline':'Skyline','double_aspect-1_chebyshev':'D1-CHEB','double_aspect-2_chebyshev':'D2-CHEB',
                            'double_aspect-3_chebyshev':'D3-CHEB', 'double_aspect-1_manhattan':'D1-MANH', 'double_aspect-2_manhattan':'D2-MANH',
                            'double_aspect-3_manhattan': 'D3-MANH', 'equispaced_samerange_aspect-1_manhattan':'E1-MANH','equispaced_samerange_aspect-2_manhattan':'E2-MANH',
                            'equispaced_samerange_aspect-3_manhattan': 'E3-MANH', 'equispaced_samerange_aspect-1_chebyshev':'E1-CHEB', 'equispaced_samerange_aspect-2_chebyshev':'E2-CHEB',
                            'equispaced_samerange_aspect-3_chebyshev': 'E3-CHEB', 'equispaced_samerange_aspect-1_euclidean':'E1-EUCL', 'equispaced_samerange_aspect-2_euclidean':'E2-EUCL',
                            'equispaced_samerange_aspect-3_euclidean': 'E3-EUCL', 'double_aspect-1_euclidean':'D1-EUCL', 'double_aspect-2_euclidean':'D2-EUCL',
            ...
            for qid in top5:
                for doc_pos, document in enumerate(dict_top_ranks5_per_topic[metric][qid]):
                    if document in qrels_dict[qid]:
                        # print(qid, np.sum([int(x) for x in qrels_dict[qid][document]]), doc_pos+1, sep="#")
                        print(qid, np.sum([int(x) for x in qrels_dict[qid][document]]), doc_pos+1, sep="#", file=file_out)
                    else:
                        # print(qid, '0', doc_pos+1, sep="#")
